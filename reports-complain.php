    ...
    <div class="container">
                ...
                <div class="upper">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-content">
                            ...
                            <div>
                                <a href="reports-complain.php" id="active">Complain</a>
                            </div>
                            <div>
        ...
        <main>
            <header>
                <div class="title">
                    <div class="text">
                        <h1>Incident Reports - Complain</h1>
                    </div>
                </div>
                <div class="options">
...
// Construct the query
$query = "SELECT cr.*, r.first_name, r.last_name, r.phone_number FROM complaint_reports cr LEFT JOIN reporters r ON cr.id = r.report_id WHERE 1";

// Add search condition if search term is provided
if (!empty($search)) {
    $query .= " AND (cr.reason LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR r.first_name LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR r.last_name LIKE '%" . mysqli_real_escape_string($conn, $search) . "%')";
}

// Add filter condition if filter is provided
if (!empty($filter)) {
    $query .= " AND cr.status = '" . mysqli_real_escape_string($conn, $filter) . "'";
}
...
if (mysqli_num_rows($result) > 0) {
    ...
    <a href='php/complaint_pdf.php?id=" . htmlspecialchars($row['id']) . "' class='actionButton' target='_blank'>View</a>
...
?>
    ...
    if (confirm('Are you sure you want to verify this report?')) {
        fetch(`php/get_reportcomplain_status.php?id=${reportId}`, {
            method: 'GET',
            headers: {
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                fetch(`php/update_status_complain.php?id=${reportId}`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json'
                    }
...
document.getElementById('searchBtn').addEventListener('click', () => {
    ...
    window.location.href = `reports-complain.php?search=${encodeURIComponent(searchValue)}&filter=${encodeURIComponent(filterValue)}`;
});
