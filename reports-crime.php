                ...
                <div class="upper">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-content">
                            <div>
                                <a href="reports-crime.php" id="active">Crime</a>
                            </div>
                            <div>
...
// Construct the query
$query = "SELECT cr.*, rc.first_name, rc.last_name, rc.phone_number FROM crime_reports cr LEFT JOIN reporters_crime rc ON cr.id = rc.report_id WHERE 1";

// Add search condition if search term is provided
if (!empty($search)) {
    $query .= " AND (cr.crimeType LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR rc.first_name LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR rc.last_name LIKE '%" . mysqli_real_escape_string($conn, $search) . "%')";
}

// Add filter condition if filter is provided
if (!empty($filter)) {
    $query .= " AND cr.crimeType = '" . mysqli_real_escape_string($conn, $filter) . "'";
}
...
if (mysqli_num_rows($result) > 0) {
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        echo "<td>" . htmlspecialchars($row['crimeType']) . "</td>";
    ...
    <a href='php/crime_pdf.php?id=" . htmlspecialchars($row['id']) . "' class='actionButton' target='_blank'>View</a>
...
?>
    ...
    if (confirm('Are you sure you want to delete this report?')) {
        fetch(`delete-crime.php?id=${userId}`, {
    method: 'GET',
    headers: {
        ...
        .then(data => {
            if (data.success) {
                ...
                alert('Failed to delete the report: ' + data.message);
            }
        })
        .catch(error => {
  ...
  function sendReport(reportId) {
    if (confirm('Are you sure you want to verify this report?')) {
        fetch(`php/get_reportcrime_status.php?id=${reportId}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json'
            }
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                fetch(`php/update_status_crime.php?id=${reportId}`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json'
                    }
...
document.getElementById('searchBtn').addEventListener('click', () => {
    ...
    window.location.href = `reports-crime.php?search=${encodeURIComponent(searchValue)}&filter=${encodeURIComponent(filterValue)}`;
});
