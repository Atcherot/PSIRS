    ...
    <div class="container">
                ...
                <div class="upper">
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-content">
                            ...
                            <div>
                                <a href="reports-incident.php" id="active">Incident</a>
                            </div>
                            <div>
        ...
        <main>
            <header>
                <div class="title">
                    <div class="text">
                        <h1>Incident Reports - Incident</h1>
                    </div>
                </div>
                <div class="options">
...
// Construct the query
$query = "SELECT ir.*, ri.first_name, ri.last_name, ri.phone_number FROM incident_reports ir LEFT JOIN reporters_incident ri ON ir.id = ri.report_id WHERE 1";

// Add search condition if search term is provided
if (!empty($search)) {
    $query .= " AND (ir.incident_type LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR ri.first_name LIKE '%" . mysqli_real_escape_string($conn, $search) . "%' OR ri.last_name LIKE '%" . mysqli_real_escape_string($conn, $search) . "%')";
}

// Add filter condition if filter is provided
if (!empty($filter)) {
    $query .= " AND ir.status = '" . mysqli_real_escape_string($conn, $filter) . "'";
}
...
if (mysqli_num_rows($result) > 0) {
    ...
    <a href='php/incident_pdf.php?id=" . htmlspecialchars($row['id']) . "' class='actionButton' target='_blank'>View</a>
...
?>
    ...
    if (confirm('Are you sure you want to delete this report?')) {
        fetch(`delete-incident.php?id=${userId}`, {
    method: 'GET',
    headers: {
...
function sendReport(reportId) {
    if (confirm('Are you sure you want to verify this report?')) {
        fetch(`php/get_report_status.php?id=${reportId}`, {
            method: 'GET',
            headers: {
                'Accept': 'application/json'
            }
        ...
        .then(data => {
            ...
            if (data.success) {
                ...
                fetch(`php/update_status_incident.php?id=${reportId}`, {
                    method: 'GET',
                    headers: {
                        'Accept': 'application/json'
                    }
...
document.getElementById('searchBtn').addEventListener('click', () => {
    ...
    window.location.href = `reports-incident.php?search=${encodeURIComponent(searchValue)}&filter=${encodeURIComponent(filterValue)}`;
});
